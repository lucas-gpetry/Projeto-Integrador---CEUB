...
**Observa-se que há uma tendência à linearidade quanto ao total de casos registrados por dia da semana**
Isso sugere que ocorrências nesse contexto não estão restritas a datas comemorativas ou finais de semana. Pelo contrário, a ausência de sazonalidade indica que o consumo de álcool por parte dos agressores é constante
```{python}
...
bar_vertical_temporal_highlight(
    ...
    x="dia_semana",
    y="percentual",
    title="% de ocorrências com consumo de álcool por dia da semana",
    COLORS=COLORS
)
...
bar_vertical_temporal_highlight(
    ...
    x="dia_semana",
    y="percentual",
    title="% de ocorrências com descumprimento de medida protetiva por dia da semana",
    COLORS=COLORS
)
```
...
Encontrar maneira de localizar reincidências. Clusterizar as latitudes e longitudes e buscar por um raio de 30-50 metros é uma possibilidade para escapar das pequenas variações de GPS
